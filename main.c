...
int chooseroom(int choice, int front)
{
 	...
 	if (choice==1)
 	{
 	 	for (i=0; i<10; i++)
 	 	{
 	 	 	...
 	 	 	{
 	 	 	 	Qblock_1bed[i]=front;
 	 	 	 	printf("CONGRATULATIONS !! Student Rank %d has been alloted 1 bed AC room in Kurunji block.",present[front]);
                allot=1;
 	 	 	 	break;
 	 	 	}
 	 	}
 	 	if (allot==0)
 	 	 	printf("\nSorry, Kurunji Block is full.");
 	}
    if (choice==2)
    {
	 	z:
 	 	...
 	 	for (i=0; i<10; i++)
 	 	{
 	 	 	...
 	 	 	{
 	 	 	 	Qblock_2bed[i]=front;
 	 	 	 	printf("CONGRATULATIONS !! Student Rank %d alloted  bed AC room  in Kurunji Block", present[front]);
 	 	 	 	allot=1;
                break;
 	 	 	}
 	 	}
 	 	if (allot==0)
 	 	 	printf("Sorry, Kurunji Block is full");
 	}
 	return allot;
}
...
int main()
{
    ...
    printf("\t>>>   CEG STUDENT ACCOMODATION  <<<\n");
    ...
    while(counter<=12)
    {
        printf("\n1.Get Details from CEG Database\n2.Search\n3.Display Students\n4.Calculate NCGPA\n5.Exit\nEnter your choice : ");
        scanf("%d",&opt);
        switch(opt)
        {
        case 1:
    ...
    if(opt!=1)
        exit(0);
    system("cls");
    printf(">>>>  WELCOME TO CEG HOSTEL ROOM COUNSELLING  <<<<\n");
    printf("\nProceed for registration:\n");
    registration(total);
    printf("\t\tROOMS AVAILABLE\n");
    printf("\tROOM TYPE\t\tTOTAL ROOMS\tCHOICE\n");
    printf("   Kurunji BLOCK 1 BED AC\t\t\t10\t  1\n   Kurunji BLOCK 2 BED AC \t\t\t10\t  2\n");
    int front=1;
